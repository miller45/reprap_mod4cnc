...
volatile uint8_t *wait_signal_reg;
volatile uint8_t wait_signal_bitmask;

volatile uint8_t *wait_led_reg;
volatile uint8_t wait_led_bitmask;

volatile bool cantStepLEDIsOn;
...
*/  
  ...
  for (unsigned char i = 0; i < 3; i++) {
    ...
    pinMode(min_pins[i], INPUT_PULLUP);
#endif
#if ENDSTOPS_MAX_ENABLED == 1
    pinMode(max_pins[i], INPUT_PULLUP);
#endif
...
#ifdef CANTSTEP_LED_PIN
    pinMode(CANTSTEP_LED_PIN,OUTPUT);
...
void disable_steppers_slow()
{
//  Serial.println("disable steppers slow");
  unsigned int enable_pins[3] = { X_ENABLE_PIN, Y_ENABLE_PIN, Z_ENABLE_PIN };
  for (unsigned int i = 0; i < 3; i++) {
    digitalWrite(enable_pins[i],!ENABLE_ON);
  }
  delay(1);
}
...
if(cantStepLEDIsOn){
//   digitalWrite(CANTSTEP_LED_PIN,LOW);
//   cantStepLEDIsOn=false;
}
#if ENDSTOPS_MIN_ENABLED == 1
#if ENDSTOPS_INVERTING == 1
  if (!(*min_input_reg[axis] & min_bitmask[axis]) && !direction){
//    digitalWrite(CANTSTEP_LED_PIN,HIGH);
//    cantStepLEDIsOn=true;
    return false;
  }
#else
...
#if ENDSTOPS_MAX_ENABLED == 1
#if ENDSTOPS_INVERTING == 1
  if (!(*max_input_reg[axis] & max_bitmask[axis]) && direction){
//     digitalWrite(CANTSTEP_LED_PIN,HIGH);
//     cantStepLEDIsOn=true;
     return false;
  }
...
//void set_and_wait_for_signal(){
//   wait_for_signal=true;
//   //hackscan
////  if ((*wait_signal_reg & wait_signal_bitmask)){
//   fastDigitalWrite( wait_led_reg, wait_led_bitmask,HIGH);
//   
//   //while(!(*wait_signal_reg & wait_signal_bitmask)){
//  //wait for down
//  while(digitalRead(WAIT_SIGNAL_PIN)==LOW){
//      delay(1);
//  }
// //wait for release
//  while(digitalRead(WAIT_SIGNAL_PIN)==HIGH){
//      delay(1);
//  }
//  delay(500);
//  wait_for_signal=false;
//  fastDigitalWrite( wait_led_reg, wait_led_bitmask,LOW);
//}




void setMicroStep(int stepMode){
   if(stepMode==1){
